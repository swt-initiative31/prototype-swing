...
// * Selection, Verify, VerifyKey, OrientationChange
// * </dl>
// * <p>
...
//
//	static final Comparator<Point> SELECTION_COMPARATOR = Comparator.comparingInt(StyledText::getX)
...
//	Listener listener;
//	TextChangeListener textChangeListener; // listener for TextChanging, TextChanged and TextSet events from
//											// StyledTextContent
...
//	int topIndexY;
//	int clientAreaHeight = 0; // the client area height. Needed to calculate content width for new visible
//								// lines during Resize callback
//	int clientAreaWidth = 0; // the client area width. Needed during Resize callback to determine if line
//								// wrap needs to be recalculated
...
//	int bottomMargin;
//	Color marginColor;
//	int columnX; // keep track of the horizontal caret position when changing lines/pages. Fixes
//					// bug 5935
//	Caret[] carets;
//	int[] caretOffsets = { 0 };
//	int caretAlignment;
//	Point[] selection = { new Point(0, 0) }; // x and y are start and end caret offsets of selection (x <= y)
//	int[] selectionAnchors = { 0 }; // position of selection anchor for the selection ranges. 0 based offset from
//									// beginning of text
...
//	int textLimit = -1; // limits the number of characters the user can type in the widget. Unlimited by
//						// default.
...
//	 * <code>Printing</code> is returned in the StyledText#print(Printer) API. The
//	 * run() method may be invoked from any thread.
//	 */
...
//
//		/**
//		 * Creates an instance of <code>Printing</code>. Copies the widget content and
//		 * rendering data that needs to be requested from listeners.
//		 *
...
//		 *
//		 * @param printerContent <code>StyledTextContent</code> to request line data
//		 *                       for.
//		 */
...
//		 *
//		 * @param page   page number to print, if specified in the
...
//
//		/**
//		 * Print one segment of a header or footer decoration. Headers and footers have
//		 * three different segments. One each for left aligned, centered, and right
//		 * aligned text.
//		 *
//		 * @param segment   decoration segment to print
//		 * @param alignment alignment of the segment. 0=left, 1=center, 2=right
//		 * @param page      page number to print, if specified in the decoration
//		 *                  segment.
//		 * @param header    true = print the header, false = print the footer
//		 */
...
//		// set the fg in the OS to ensure that these are the same as StyledText,
//		// necessary
//		// for ensuring that the bg/fg the IME box uses is the same as what StyledText
//		// uses
...
//					return;
//				setCaretLocations(Arrays.stream(caretOffsets).mapToObj(this::getPointAtOffset).toArray(Point[]::new),
//						direction);
//			};
...
//	 * @exception IllegalArgumentException
//	 *                                     <ul>
//	 *                                     <li>ERROR_NULL_ARGUMENT when listener is
//	 *                                     null</li>
//	 *                                     </ul>
//	 */
...
//	 * <p>
//	 * A BidiSegmentEvent is sent whenever a line of text is measured or rendered.
//	 * You can specify text ranges in the line that should be treated as if they had
//	 * a different direction than the surrounding text. This may be used when
//	 * adjacent segments of right-to-left text should not be reordered relative to
//	 * each other. E.g., multiple Java string literals in a right-to-left language
//	 * should generally remain in logical order to each other, that is, the way they
//	 * are stored.
//	 * </p>
//	 *
...
//
//	/**
//	 * Adds a modify listener. A Modify event is sent by the widget when the widget
//	 * text has changed.
//	 *
...
//
//	/**
//	 * Adds a paint object listener. A paint object event is sent by the widget when
//	 * an object needs to be drawn.
//	 *
//	 * @param listener the listener
//	 * @exception SWTException
//	 *                                     <ul>
...
//	 * the start and end caret indices of the selection[0]. The selection values
//	 * returned are visual (i.e., x will always always be &lt;= y). No event is sent
//	 * when the caret is moved while the selection length is 0.
//	 * </p>
//	 * <p>
...
//
//	/**
//	 * Adds a verify key listener. A VerifyKey event is sent by the widget when a
//	 * key is pressed. The widget ignores the key press if the listener sets the
...
//
//	/**
//	 * Adds a verify listener. A Verify event is sent by the widget when the widget
//	 * text is about to change. The listener can set the event text and the doit
//	 * field to change the text that is set in the widget or to force the widget to
//	 * ignore the text change.
//	 *
...
//
//	/**
//	 * Adds a word movement listener. A movement event is sent when the boundary of
//	 * a word is needed. For example, this occurs during word next and word previous
//	 * actions.
//	 *
...
//
//	/**
//	 * Calculates the top index based on the current vertical scroll offset. The top
//	 * index is the index of the topmost fully visible line or the topmost partially
//	 * visible line if no line is fully visible. The top index starts at 0.
//	 */
...
//				}
//				if (lineIndex < lineCount && -delta + renderer.getCachedLineHeight(lineIndex) <= clientAreaHeight
...
//					lineIndex--;
//				}
//				if (lineIndex == 0 || -delta + renderer.getCachedLineHeight(lineIndex) <= clientAreaHeight - topMargin
//						- bottomMargin) {
//					topIndex = lineIndex;
//					topIndexY = -delta;
//				} else {
...
//		}
//		if (topIndex < 0) {
//			// TODO: This logging is in place to determine why topIndex is getting set to
//			// negative values.
//			// It should be deleted once we fix the root cause of this issue. See bug 487254
//			// for details.
//			System.err.println("StyledText: topIndex was " + topIndex + ", isFixedLineHeight() = " + isFixedLineHeight()
//					+ ", delta = " + delta + ", content.getLineCount() = " + content.getLineCount()
//					+ ", clientAreaHeight = " + clientAreaHeight + ", oldTopIndex = " + oldTopIndex
//					+ ", oldTopIndexY = " + oldTopIndexY + ", getVerticalScrollOffset = " + getVerticalScrollOffset()
//					+ ", oldDelta = " + oldDelta + ", getVerticalIncrement() = " + getVerticalIncrement());
//			topIndex = 0;
//		}
...
//	 * The <code>DND.CLIPBOARD</code> clipboard is used for data that is transferred
//	 * by keyboard accelerator (such as Ctrl+C/Ctrl+V) or by menu action.
//	 * </p>
//	 *
//	 * @exception SWTException
//	 *                         <ul>
...
//
//	/**
//	 * Copies the selected text to the specified clipboard. The text will be put in
//	 * the clipboard in plain text, HTML, and RTF formats.
//	 * <p>
//	 * The clipboardType is one of the clipboard constants defined in class
//	 * <code>DND</code>. The <code>DND.CLIPBOARD</code> clipboard is used for data
//	 * that is transferred by keyboard accelerator (such as Ctrl+C/Ctrl+V) or by
//	 * menu action. The <code>DND.SELECTION_CLIPBOARD</code> clipboard is used for
//	 * data that is transferred by selecting text and pasting with the middle mouse
//	 * button.
//	 * </p>
//	 *
...
//							new Transfer[] { TextTransfer.getInstance() }, type);
//					return true;
//				}
//			} else {
...
//
//	/**
//	 * Returns the Always Show Scrollbars flag. True if the scrollbars are always
//	 * shown even if they are not required. False if the scrollbars are only visible
//	 * when some part of the content needs to be scrolled to be seen. The H_SCROLL
//	 * and V_SCROLL style bits are also required to enable scrollbars in the
...
//	 * </p>
//	 *
//	 * @param text the text that may have line delimiters that don't match the model
//	 *             line delimiter. Possible line delimiters are CR ('\r'), LF
//	 *             ('\n'), CR/LF ("\r\n")
//	 * @return the converted text that only uses the line delimiter specified by the
//	 *         model. Returns only the first line if the widget has the SWT.SINGLE
//	 *         style.
//	 */
...
//
//	/**
//	 * Create the bitmaps to use for the caret in bidi mode. This method only needs
//	 * to be called upon widget creation and when the font changes (the caret bitmap
//	 * height needs to match font height).
//	 */
...
//
//	/**
//	 * A mouse move event has occurred. See if we should start autoscrolling. If the
//	 * move position is outside of the client area, initiate autoscrolling.
...
//				public void run() {
//					/*
//					 * Bug 437357 - NPE in StyledText.getCaretLine StyledText.content is null at
//					 * times, probably because the widget itself has been disposed.
//					 */
//					if (isDisposed())
//						return;
...
//	 * <p>
//	 * If a carriage return was typed replace it with the line break character used
//	 * by the widget on this platform.
//	 * </p>
//	 *
...
//
//	/**
//	 * Moves the caret to the end of the selection if a selection exists. Otherwise,
//	 * if no selection exists move the cursor according to the cursor selection
//	 * rules.
//	 *
...
//
//	/**
//	 * Moves the caret one line down and to the same character offset relative to
//	 * the beginning of the line. Move the caret to the end of the new line if the
//	 * new line is shorter than the character offset. Moves the caret to the end of
//	 * the text if the caret already is on the last line.
//	 */
...
//
//	/**
//	 * Moves the caret one line up and to the same character offset relative to the
//	 * beginning of the line. Move the caret to the end of the new line if the new
//	 * line is shorter than the character offset. Moves the caret to the beginning
//	 * of the document if it is already on the first line.
//	 */
...
//	 * @param select the location change is a selection operation. include the line
...
//				|| newCaretLine != content.getLineAtOffset(caretOffsets[0]);
...
//		if (caretOffsets[0] <= selection[0].x || (caretOffsets[0] > selection[0].x && caretOffsets[0] < selection[0].y
//				&& selectionAnchors[0] == selection[0].x)) {
//			doSelection(ST.COLUMN_PREVIOUS);
//		} else {
//			doSelection(ST.COLUMN_NEXT);
//		}
...
//	 * selection extends from high index to low index (i.e., right to left, or caret
//	 * is at left border of selection on non-bidi platforms) the start offset of the
//	 * word preceding the selection is returned. If the current selection extends
//	 * from low index to high index the end offset of the word following the
...
//	 * <p>
//	 * The caret is scrolled the same number of lines so that its location relative
//	 * to the top line remains the same. The exception is the end of the text where
//	 * a full page scroll is not possible. In this case the caret is moved after the
//	 * last character.
//	 * </p>
//	 *
...
//	 * current page becomes the fully visible last line. The caret is scrolled the
//	 * same number of lines so that its location relative to the top line remains
//	 * the same. The exception is the beginning of the text where a full page scroll
//	 * is not possible. In this case the caret is moved in front of the first
//	 * character.
//	 */
...
//
//	/**
//	 * Moves the caret to the next character or to the beginning of the next line if
...
//
//	/**
//	 * Moves the caret to the previous character or to the end of the previous line
...
//	/**
//	 * Moves the caret one line down and to the same character offset relative to
//	 * the beginning of the line. Moves the caret to the end of the new line if the
//	 * new line is shorter than the character offset. Moves the caret to the end of
//	 * the text if the caret already is on the last line. Adjusts the selection
//	 * according to the caret change. This can either add to or subtract from the
//	 * old selection, depending on the previous selection direction.
//	 */
...
//	/**
//	 * Moves the caret one line up and to the same character offset relative to the
//	 * beginning of the line. Moves the caret to the end of the new line if the new
//	 * line is shorter than the character offset. Moves the caret to the beginning
//	 * of the document if it is already on the first line. Adjusts the selection
//	 * according to the caret change. This can either add to or subtract from the
//	 * old selection, depending on the previous selection direction.
//	 */
...
//	 * </p>
//	 * <p>
//	 * Adjusts the selection according to the caret change. This can either add to
//	 * or subtract from the old selection, depending on the previous selection
//	 * direction.
//	 * </p>
//	 */
...
//	 * <p>
//	 * The caret is scrolled the same number of lines so that its location relative
//	 * to the top line remains the same. The exception is the beginning of the text
//	 * where a full page scroll is not possible. In this case the caret is moved in
...
//			showCaret();
//		} else {
//			int[] linesForCurrentCarets = Arrays.stream(caretOffsets).map(offset -> content.getLineAtOffset(offset))
//					.toArray();
...
//		setCaretOffsets(Arrays.stream(caretOffsets).map(offset -> getWordPrevious(offset, SWT.MOVEMENT_WORD)).toArray(),
//				OFFSET_LEADING);
//		showCaret();
//	}
//
...
//		setCaretOffsets(Arrays.stream(caretOffsets)
//				.map(offset -> getClusterPrevious(offset, content.getLineAtOffset(offset))).toArray(), SWT.DEFAULT);
//		showCaret();
//	}
//
//	/**
//	 * Moves the caret one character to the right. Do not go to the next line. When
//	 * in a bidi locale and at a R2L character the caret is moved to the end of the
//	 * R2L segment (visually left) and then one character to the right (visually
//	 * right because it's now in a L2R segment).
//	 */
//	void doVisualNext() {
//		setCaretOffsets(Arrays.stream(caretOffsets)
//				.map(offset -> getClusterNext(offset, content.getLineAtOffset(offset))).toArray(), SWT.DEFAULT);
//		showCaret();
//	}
//
//	/**
//	 * Moves the caret to the end of the next word. If a selection exists, move the
//	 * caret to the end of the selection and remove the selection.
//	 */
//	void doWordNext() {
//		if (Arrays.stream(selection).anyMatch(p -> p.x != p.y)) {
...
//	 * @exception IllegalArgumentException
//	 *                                     <ul>
//	 *                                     <li>ERROR_INVALID_RANGE when the offset
//	 *                                     is outside the valid range (&lt; 0 or
//	 *                                     &gt; getCharCount())</li>
//	 *                                     </ul>
//	 *
//	 * @since 3.2
...
//
//	/**
//	 * Returns the block selection bounds. The bounds is relative to the upper left
...
//	 *
//	 * @return content the user defined content implementation that is used for text
//	 *         storage or the default content implementation if no user defined
...
//	 *
//	 * @return horizontal scroll offset relative to the start of the line, measured
//	 *         in character increments starting at 0, if &gt; 0 the content is
//	 *         scrolled
//	 * @exception SWTException
//	 *                         <ul>
//	 *                         <li>ERROR_WIDGET_DISPOSED - if the receiver has been
...
//	 *         measured in SWT logical point starting at 0, if &gt; 0 the content is
//	 *         scrolled.
//	 * @exception SWTException
//	 *                         <ul>
//	 *                         <li>ERROR_WIDGET_DISPOSED - if the receiver has been
...
//	 *
//	 * @param key a key code defined in SWT.java or a character. Optionally ORd with
//	 *            a state mask. Preferred state masks are one or more of SWT.MOD1,
//	 *            SWT.MOD2, SWT.MOD3, since these masks account for modifier
//	 *            platform differences. However, there may be cases where using the
//	 *            specific state masks (i.e., SWT.CTRL, SWT.SHIFT, SWT.ALT,
//	 *            SWT.COMMAND) makes sense.
//	 * @return one of the predefined actions defined in ST.java or SWT.NULL if there
//	 *         is no action associated with the key.
//	 * @exception SWTException
//	 *                         <ul>
//	 *                         <li>ERROR_WIDGET_DISPOSED - if the receiver has been
//	 *                         disposed</li>
...
//
//	/**
//	 * Returns the line at the given line index without delimiters. Index 0 is the
//	 * first line of the content. When there are not any lines, getLine(0) is a
//	 * valid call that answers an empty string.
//	 *
...
//	 * @exception IllegalArgumentException
//	 *                                     <ul>
//	 *                                     <li>ERROR_INVALID_RANGE when the line
//	 *                                     index is outside the valid range (&lt; 0
//	 *                                     or &gt;= getLineCount())</li>
//	 *                                     </ul>
//	 * @since 3.4
//	 */
...
//	 * getCharCount() so that getLineAtOffset(getCharCount()) returns the line of
//	 * the insert location.
//	 *
//	 * @param offset offset relative to the start of the content. 0 &lt;= offset
//	 *               &lt;= getCharCount()
...
//
//	/**
//	 * Returns the background color of the line at the given index. Returns null if
//	 * a LineBackgroundListener has been set or if no background color has been
//	 * specified for the line. Should not be called if a LineBackgroundListener has
//	 * been set since the listener maintains the line background colors.
//	 *
//	 * @param index the index of the line
...
//	 *
//	 * @param lineOffset offset of the line start relative to the start of the
//	 *                   content.
...
//
//	/**
//	 * Returns the number of lines that can be completely displayed in the widget
//	 * client area.
//	 *
//	 * @return number of lines that can be completely displayed in the widget client
//	 *         area.
//	 */
...
//
//	/**
//	 * Returns the line delimiter used for entering new lines by key down or paste
//	 * operation.
//	 *
...
//	 * line. Clamps out of ranges index.
//	 *
//	 * @param lineIndex the line index, the max value is lineCount. If lineIndex ==
//	 *                  lineCount it returns the bottom SWT logical point of the
//	 *                  last line. It means this function can be used to retrieve
//	 *                  the bottom SWT logical point of any line.
//	 *
...
//
//	/**
//	 * Returns the line index for a y, relative to the client area. The line index
//	 * returned is always in the range 0..lineCount - 1.
//	 *
...
//
//	/**
//	 * Returns the x, y location of the upper left corner of the character bounding
//	 * box at the specified offset in the text. The point is relative to the upper
//	 * left corner of the widget client area.
//	 *
//	 * @param offset offset relative to the start of the content. 0 &lt;= offset
//	 *               &lt;= getCharCount()
//	 * @return x, y location of the upper left corner of the character bounding box
//	 *         at the specified offset in the text.
//	 * @exception SWTException
//	 *                                     <ul>
//	 *                                     <li>ERROR_WIDGET_DISPOSED - if the
//	 *                                     receiver has been disposed</li>
...
//
//	/**
//	 * Returns <code>true</code> if the mouse navigator is enabled. When mouse
//	 * navigator is enabled, the user can navigate through the widget by pressing
//	 * the middle button and moving the cursor
//	 *
...
//	 *
//	 * @param lineIndex index of the line, 0 based relative to the first line in the
//	 *                  content. 0 &lt;= lineIndex &lt; getLineCount(), except
//	 *                  lineIndex may always be 0
//	 * @return offset offset of the first character of the line, relative to the
//	 *         beginning of the document. The first character of the document is at
...
//	 * <p>
//	 * The return value reflects the character offset that the caret will be placed
//	 * at if a mouse click occurred at the specified location. If the x coordinate
//	 * of the location is beyond the center of a character the returned offset will
//	 * be behind the character.
//	 * </p>
//	 *
//	 * @param point the origin of character bounding box relative to the origin of
//	 *              the widget client area.
//	 * @return offset of the character at the given location relative to the first
...
//	 * @exception IllegalArgumentException
//	 *                                     <ul>
//	 *                                     <li>ERROR_NULL_ARGUMENT when point is
//	 *                                     null</li>
//	 *                                     <li>ERROR_INVALID_ARGUMENT when there is
//	 *                                     no character at the specified
//	 *                                     location</li>
//	 *                                     </ul>
//	 *
//	 * @deprecated Use {@link #getOffsetAtPoint(Point)} instead for better
//	 *             performance
//	 */
//	@Deprecated
...
//
//	/**
//	 * Returns the offset of the character at the given point relative to the first
//	 * character in the document.
//	 * <p>
//	 * The return value reflects the character offset that the caret will be placed
//	 * at if a mouse click occurred at the specified point. If the x coordinate of
//	 * the point is beyond the center of a character the returned offset will be
//	 * behind the character.
//	 * </p>
//	 * Note: This method is functionally similar to
//	 * {@link #getOffsetAtLocation(Point)} except that it does not throw an
//	 * exception when no character is found and thus performs faster.
//	 *
//	 * @param point the origin of character bounding box relative to the origin of
//	 *              the widget client area.
//	 * @return offset of the character at the given point relative to the first
//	 *         character in the document. -1 when there is no character at the
//	 *         specified location.
//	 * @exception SWTException
//	 *                                     <ul>
//	 *                                     <li>ERROR_WIDGET_DISPOSED - if the
...
//
//	/**
//	 * Returns the content in the specified range using the platform line delimiter
//	 * to separate lines.
//	 *
//	 * @param writer the TextWriter to write line text into
//	 * @return the content in the specified range using the platform line delimiter
//	 *         to separate lines as written by the specified TextWriter.
//	 */
...
//
//	/**
//	 * Returns all the ranges of text that have an associated StyleRange. Returns an
//	 * empty array if a LineStyleListener has been set. Should not be called if a
//	 * LineStyleListener has been set since the listener maintains the styles.
//	 * <p>
//	 * The ranges array contains start and length pairs. Each pair refers to the
//	 * corresponding style in the styles array. For example, the pair that starts at
//	 * ranges[n] with length ranges[n+1] uses the style at styles[n/2] returned by
//	 * <code>getStyleRanges(int, int, boolean)</code>.
//	 * </p>
//	 *
...
//	 * @exception IllegalArgumentException
//	 *                                     <ul>
//	 *                                     <li>ERROR_INVALID_RANGE if start or
//	 *                                     length are outside the widget
//	 *                                     content</li>
//	 *                                     </ul>
//	 *
//	 * @since 3.2
...
//	 * that contains N characters, there are N+1 caret positions, ranging from 0..N
//	 * </p>
//	 * <p>
...
//	 *         character, y is the offset after the last selected character. The
//	 *         selection values returned are visual (i.e., x will always always be
//	 *         &lt;= y). To determine if a selection is right-to-left (RtoL) vs.
//	 *         left-to-right (LtoR), compare the caretOffset to the start and end of
//	 *         the selection (e.g., caretOffset == start of selection implies that
//	 *         the selection is RtoL).
...
//	 *         returned are visual (i.e., length will always always be positive). To
//	 *         determine if a selection is right-to-left (RtoL) vs. left-to-right
//	 *         (LtoR), compare the caretOffset to the start and end of the selection
//	 *         (e.g., caretOffset == start of selection implies that the selection
//	 *         is RtoL).
//	 * @exception SWTException
//	 *                         <ul>
//	 *                         <li>ERROR_WIDGET_DISPOSED - if the receiver has been
...
//
//	/**
//	 * Returns the selected ranges of text. If block is enabled, return the ranges
//	 * that are inside the block selection rectangle.
//	 * <p>
//	 * The ranges array contains start and length pairs.
//	 * <p>
//	 * When the receiver is not in block selection mode the return arrays contains
//	 * the start and length of the regular selections.
//	 *
...
//		}
//		return Arrays.stream(selection).map(sel -> content.getTextRange(sel.x, sel.y - sel.x))
...
//
//	/**
//	 * @see #getBidiSegments Supports deprecated setBidiColoring API. Remove when
//	 *      API is removed.
//	 */
...
//	 * <p>
//	 * Returns null if a LineStyleListener has been set or if a style is not set for
//	 * the offset. Should not be called if a LineStyleListener has been set since
//	 * the listener maintains the styles.
//	 * </p>
//	 *
//	 * @param offset the offset to return the style for. 0 &lt;= offset &lt;
//	 *               getCharCount() must be true.
//	 * @return a StyleRange with start == offset and length == 1, indicating the
//	 *         style at the given offset. null if a LineStyleListener has been set
...
//	 * @exception IllegalArgumentException
//	 *                                     <ul>
//	 *                                     <li>ERROR_INVALID_ARGUMENT when the
//	 *                                     offset is invalid</li>
//	 *                                     </ul>
//	 */
...
//	 * called if a LineStyleListener has been set since the listener maintains the
//	 * styles.
//	 * </p>
//	 * <p>
...
//	 * <code>getStyleRanges(boolean)</code> can be used to get the styles without
//	 * the ranges.
//	 * </p>
//	 *
...
//	 * </p>
//	 * <p>
//	 * Note: When <code>includeRanges</code> is true, the start and length fields of
//	 * each StyleRange will be valid, however the StyleRange objects may need to be
//	 * cloned. When <code>includeRanges</code> is false,
//	 * <code>getRanges(int, int)</code> can be used to get the associated ranges.
//	 * </p>
//	 *
//	 * @param includeRanges whether the start and length field of the StyleRanges
//	 *                      should be set.
//	 *
//	 * @return the styles or an empty array if a LineStyleListener has been set.
//	 *
...
//	 * </p>
//	 * <p>
//	 * Note: Because the StyleRange includes the start and length, the same instance
//	 * cannot occur multiple times in the array of styles. If the same style
//	 * attributes, such as font and color, occur in multiple StyleRanges,
//	 * <code>getStyleRanges(int, int, boolean)</code> can be used to get the styles
//	 * without the ranges.
//	 * </p>
//	 *
//	 * @param start  the start offset of the style ranges to return
//	 * @param length the number of style ranges to return
//	 *
//	 * @return the styles or an empty array if a LineStyleListener has been set. The
//	 *         returned styles will reflect the given range. The first returned
//	 *         <code>StyleRange</code> will have a starting offset &gt;= start and
//	 *         the last returned <code>StyleRange</code> will have an ending offset
//	 *         &lt;= start + length - 1
//	 *
//	 * @exception SWTException
//	 *                                     <ul>
...
//	 * @exception IllegalArgumentException
//	 *                                     <ul>
//	 *                                     <li>ERROR_INVALID_RANGE when start and/or
//	 *                                     end are outside the widget content</li>
//	 *                                     </ul>
//	 *
//	 * @see #getStyleRanges(int, int, boolean)
//	 *
...
//
//	/**
//	 * Returns the smallest bounding rectangle that includes the characters between
//	 * two offsets.
//	 *
...
//	 * Gets the top index.
//	 * <p>
//	 * The top index is the index of the fully visible line that is currently at the
//	 * top of the widget or the topmost partially visible line if no line is fully
//	 * visible. The top index changes when the widget is scrolled. Indexing is zero
//	 * based.
//	 * </p>
//	 *
...
//	 * <p>
//	 * The top point is the SWT logical point position of the line that is currently
//	 * at the top of the widget. The text widget can be scrolled by points by
//	 * dragging the scroll thumb so that a partial line may be displayed at the top
...
//					if (wordWrap && !lineBegin && (Arrays.binarySearch(caretOffsets, offset) < 0
//							|| Arrays.stream(selection).allMatch(p -> p.x == p.y))) {
//						int[] offsets = layout.getLineOffsets();
//						for (int i : offsets) {
//							if (i == offsetInLine) {
//								lineBegin = true;
...
//	 * @exception IllegalArgumentException
//	 *                                     <ul>
//	 *                                     <li>ERROR_NULL_ARGUMENT when string is
//	 *                                     null</li>
//	 *                                     </ul>
//	 */
...
//
//		/*
//		 * Redraw end of line before start line if wrapped and start offset is first
//		 * char
//		 */
...
//				ignore = event.stateMask == SWT.ALT || event.stateMask == SWT.CTRL
//						|| event.stateMask == (SWT.ALT | SWT.SHIFT) || event.stateMask == (SWT.CTRL | SWT.SHIFT);
//			}
...
//
//	/**
//	 * If a VerifyKey listener exists, verify that the key that was entered should
//	 * be processed.
//	 *
//	 * @param event keyboard event
//	 */
...
//		if (event.button == 2) {
//			// On GTK, if mouseNavigator is enabled we have to distinguish a short
//			// middle-click (to paste content) from
...
//					if (event.x > offsetPoint.x && offset < Math.min(max, lineOffset + lineLength) // Not beyond EOL
...
//
//	/**
//	 * Updates the caret location and selection if mouse button 1 is pressed during
//	 * the mouse move.
//	 */
...
//					} else {
//						gc.drawRectangle(caret.getBounds().x, caret.getBounds().y, caret.getBounds().width,
//								getLineHeight(caretOffsets[i]));
//					}
//				}
//			}
//		}
...
//			setScrollBars(true);
//			claimRightFreeSpace();
//			// StyledText allows any value for horizontalScrollOffset when clientArea is
//			// zero
...
//
//	/**
//	 * Updates the caret position and selection and the scroll bars to reflect the
//	 * content change.
//	 */
//	void handleTextChanged(TextChangedEvent event) {
//		int offset = ime.getCompositionOffset();
//		if (offset != -1 && lastTextChangeStart < offset) {
//			// updating the compositionOffset of the ongoing IME if last IMEs text is
//			// deleted meanwhile
//			// for example 1.insert IME text 2.open IME menu 3. ctrl+backspace => 4. commit
//			// IME would result in IllegalArgumentException
//			int compositionOffset = ime.getCaretOffset() + lastTextChangeNewCharCount - lastTextChangeReplaceCharCount;
//			// workaround: while ongoing IME non-ime text is deleted it may result in too
//			// big compositionOffset
//			// for example 1.insert IME text 2.insert space 3.open IME menu 4.
//			// ctrl+backspace twice => 5. commit IME would result in
...
//	 * @param event .newText text that is going to be inserted or empty String if no
...
//		int tooBigOffsets = 0;
//		while (tooBigOffsets < caretOffsets.length
//				&& caretOffsets[caretOffsets.length - 1 - tooBigOffsets] > newEndOfText) {
//			tooBigOffsets++;
//		}
//		if (tooBigOffsets != 0) {
...
//
//	/**
//	 * Called when the widget content is set programmatically, overwriting the old
//	 * content. Resets the caret position, selection and scroll offsets.
...
//	 * when the widget is in single line mode or in multi line and non-editable mode
//	 * . When in editable multi line mode we want to prevent the tab traversal and
//	 * receive the tab key event instead.
//	 *
...
//				}
//
//				// If no override info available, use defaults. Don't supply default colors,
//				// though.
//				if (e.textStyle == null) {
//					e.textStyle = new TextStyle(st.getFont(), st.foreground, st.background);
//				} else {
...
//
//	/**
//	 * Returns whether the widget is mirrored (right oriented/right to left writing
//	 * order).
//	 *
...
//
//	/**
//	 * Returns <code>true</code> if any text in the widget is selected, and
...
//
//	/**
//	 * Sends the specified verify event, replace/insert text as defined by the event
//	 * and send a modify event.
//	 *
...
//	 *                    </ul>
//	 * @param updateCaret whether or not he caret should be set behind the new text
//	 */
...
//	 * clipboard or, if there is no selection, inserts the text at the current caret
//	 * offset. If the widget has the SWT.SINGLE style and the clipboard text
//	 * contains more than one line, only the first line without line delimiters is
...
//	 * @exception IllegalArgumentException
//	 *                                     <ul>
//	 *                                     <li>ERROR_NULL_ARGUMENT when printer is
//	 *                                     null</li>
//	 *                                     </ul>
//	 */
...
//	 * @exception IllegalArgumentException
//	 *                                     <ul>
//	 *                                     <li>ERROR_NULL_ARGUMENT when printer or
//	 *                                     options is null</li>
//	 *                                     </ul>
//	 * @since 2.1
//	 */
...
//	 * <code>LineStyleListener</code> is used a redraw call is the only notification
//	 * to the widget that styles have changed and that the content width may have
//	 * changed.
//	 * </p>
//	 *
//	 * @exception SWTException
...
//	 * that area of the receiver will be painted. If the <code>all</code> flag is
//	 * <code>true</code>, any children of the receiver which intersect with the
//	 * specified area will also paint their intersecting areas. If the
//	 * <code>all</code> flag is <code>false</code>, the children will not be
//	 * painted.
//	 * <p>
//	 * Marks the content width of all lines in the specified rectangle as unknown.
//	 * Recalculates the content width of all visible lines. When a
//	 * <code>LineStyleListener</code> is used a redraw call is the only notification
//	 * to the widget that styles have changed and that the content width may have
//	 * changed.
//	 * </p>
...
//	 * @param clearBackground true if the background should be cleared as part of
//	 *                        the redraw operation. If true, the entire redraw range
//	 *                        will be cleared before anything is redrawn. If the
//	 *                        redraw range includes the last character of a line
//	 *                        (i.e., the entire line is redrawn) the line is cleared
//	 *                        all the way to the right border of the widget. The
//	 *                        redraw operation will be faster and smoother if
//	 *                        clearBackground is set to false. Whether or not the
//	 *                        flag can be set to false depends on the type of change
//	 *                        that has taken place. If font styles or background
//	 *                        colors for the redraw range have changed,
//	 *                        clearBackground should be set to true. If only
//	 *                        foreground colors have changed for the redraw range,
...
//	 * effectively deletes the styles in the given range and then adds the the new
//	 * styles.
//	 * <p>
//	 * Note: Because a StyleRange includes the start and length, the same instance
//	 * cannot occur multiple times in the array of styles. If the same style
...
//	 * @param ranges StyleRange objects containing the new style information. The
...
//	 * @exception IllegalArgumentException
//	 *                                     <ul>
//	 *                                     <li>ERROR_INVALID_RANGE when either start
//	 *                                     or end is outside the valid range (0
//	 *                                     &lt;= offset &lt;= getCharCount())</li>
//	 *                                     <li>ERROR_NULL_ARGUMENT when ranges is
//	 *                                     null</li>
//	 *                                     </ul>
//	 *
//	 * @since 2.0
...
//
//	/**
//	 * Replaces the given text range with new text. If the widget has the SWT.SINGLE
//	 * style and "text" contains more than one line, only the first line is rendered
//	 * but the text is stored unchanged. A subsequent call to getText will return
//	 * the same text that was set. Note that only a single line of text should be
//	 * set when the SWT.SINGLE style is used.
//	 * <p>
//	 * <b>NOTE:</b> During the replace operation the current selection is changed as
//	 * follows:
//	 * </p>
//	 * <ul>
...
//	 *                                     or end is outside the valid range (0
//	 *                                     &lt;= offset &lt;= getCharCount())</li>
//	 *                                     <li>ERROR_INVALID_ARGUMENT when either
//	 *                                     start or end is inside a multi byte line
//	 *                                     delimiter. Splitting a line delimiter for
//	 *                                     example by inserting text in between the
//	 *                                     CR and LF and deleting part of a line
//	 *                                     delimiter is not supported</li>
//	 *                                     <li>ERROR_NULL_ARGUMENT when string is
//	 *                                     null</li>
//	 *                                     </ul>
//	 */
...
//	 * Scrolls the widget horizontally.
//	 *
//	 * @param pixels          number of SWT logical points to scroll, &gt; 0 =
//	 *                        scroll left, &lt; 0 scroll right
//	 * @param adjustScrollBar true= the scroll thumb will be moved to reflect the
//	 *                        new scroll offset. false = the scroll thumb will not
//	 *                        be moved
//	 * @return true=the widget was scrolled false=the widget was not scrolled, the
...
//
//	/**
//	 * Returns a StyledTextEvent that can be used to request data such as styles and
//	 * background color for a line.
//	 * <p>
//	 * The specified line may be a visual (wrapped) line if in word wrap mode. The
//	 * returned object will always be for a logical (unwrapped) line.
//	 * </p>
//	 *
//	 * @param lineOffset offset of the line. This may be the offset of a visual line
//	 *                   if the widget is in word wrap mode.
//	 * @param line       line text. This may be the text of a visual line if the
//	 *                   widget is in word wrap mode.
//	 * @return StyledTextEvent that can be used to request line data for the given
//	 *         line.
//	 */
...
//	 * <code>SWT.LEFT</code>, <code>SWT.CENTER</code> or <code>SWT.RIGHT</code>. The
//	 * alignment applies for all lines.
//	 * <p>
//	 * Note that if <code>SWT.MULTI</code> is set, then <code>SWT.WRAP</code> must
//	 * also be set in order to stabilize the right edge before setting alignment.
//	 * </p>
//	 *
...
//
//	/**
//	 * Set the Always Show Scrollbars flag. True if the scrollbars are always shown
//	 * even if they are not required (default value). False if the scrollbars are
//	 * only visible when some part of the content needs to be scrolled to be seen.
//	 * The H_SCROLL and V_SCROLL style bits are also required to enable scrollbars
//	 * in the horizontal and vertical directions.
//	 *
//	 * @param show true to show the scrollbars even when not required, false to show
//	 *             scrollbars only when required
//	 *
//	 * @exception SWTException
//	 *                         <ul>
//	 *                         <li>ERROR_WIDGET_DISPOSED - if the receiver has been
...
//					return;
//				} else {
//					color = new Color(disabledBg.getRGB());
...
//	 *
//	 * @param blockSelection true=enable block selection, false=disable block
//	 *                       selection
//	 *
//	 * @since 3.5
//	 */
...
//				carets = Arrays.copyOf(carets, locations.length);
//			}
//			for (int i = Math.min(caretOffsets.length, locations.length) - 1; i >= 0; i--) { // reverse order, seee bug
//																								// 579028#c7
...
//				if (caretLine < getLineCount() && renderer.getLineHeight(caretLine) != getLineHeight()) { // word wrap,
//																											// metrics,
//																											// styles...
...
//	 *                                     <ul>
//	 *                                     <li>ERROR_INVALID_ARGUMENT when the
//	 *                                     offset is inside a multi byte line
//	 *                                     delimiter (and thus neither clearly in
//	 *                                     front of or after the line delimiter)
//	 *                                     </ul>
//	 */
...
//
//	/**
//	 * Copies the specified text range to the clipboard. The text will be placed in
//	 * the clipboard in plain text, HTML, and RTF formats.
//	 *
...
//				HTMLWriter htmlWriter = new HTMLWriter(start, length);
//				String htmlText = getPlatformDelimitedText(htmlWriter);
//				htmlText = "" + htmlText; // cause extra memory pressure to fail fast instead of failing in
//											// HTMLTransfer.javaToNative()
...
//				// Still throw as it is likely that other threads silently failed too, but at
//				// least copied text is not lost
//				throw customOome;
//			}
//		}
//		clipboard.setContents(data, types, clipboardType);
...
//	 * Overridden to handle the null case since the StyledText widget uses an ibeam
//	 * as its default cursor.
//	 *
...
//	 * <p>
//	 * <b>NOTE:</b> Italic fonts are not supported unless they have no overhang and
//	 * the same baseline as regular fonts.
//	 * </p>
//	 *
//	 * @param font new font
//	 * @exception SWTException
...
//					return;
//				} else {
//					color = new Color(disabledFg.getRGB());
...
//	 * </p>
//	 *
//	 * @param offset horizontal scroll offset relative to the start of the line,
//	 *               measured in character increments starting at 0, if equal to 0
//	 *               the content is not scrolled, if &gt; 0 = the content is
//	 *               scrolled.
//	 * @exception SWTException
//	 *                         <ul>
//	 *                         <li>ERROR_WIDGET_DISPOSED - if the receiver has been
...
//
//	/**
//	 * Sets the horizontal SWT logical point offset relative to the start of the
//	 * line. Do nothing if there is no text set.
//	 * <p>
//	 * <b>NOTE:</b> The horizontal SWT logical point offset is reset to 0 when new
//	 * text is set in the widget.
//	 * </p>
//	 *
//	 * @param pixel horizontal SWT logical point offset relative to the start of the
//	 *              line.
//	 * @exception SWTException
//	 *                         <ul>
//	 *                         <li>ERROR_WIDGET_DISPOSED - if the receiver has been
...
//	 * <p>
//	 * One action can be associated with N keys. However, each key can only have one
//	 * action (key:action is N:1 relation).
//	 * </p>
//	 *
//	 * @param key    a key code defined in SWT.java or a character. Optionally ORd
...
//	 * @param action one of the predefined actions defined in ST.java. Use SWT.NULL
...
//	 * </p>
//	 * <p>
//	 * All line attributes are maintained relative to the line text, not the line
//	 * index that is specified in this method call. During text changes, when entire
//	 * lines are inserted or removed, the line attributes that are associated with
//	 * the lines after the change will "move" with their respective text. An entire
//	 * line is defined as extending from the first character on a line to the last
//	 * and including the line delimiter.
//	 * </p>
...
//	 *                                     <ul>
//	 *                                     <li>ERROR_INVALID_ARGUMENT when the
//	 *                                     specified line range is invalid</li>
//	 *                                     </ul>
...
//		if (Arrays.stream(caretOffsets).map(content::getLineAtOffset)
//				.anyMatch(caretLine -> startLine <= caretLine && caretLine < startLine + lineCount)) {
//			setCaretLocations();
//		}
//		setAlignment();
//	}
...
//	 * background colors are discarded when setText is called. The text background
//	 * color if defined in a StyleRange overlays the line background color.
//	 * </p>
//	 * <p>
...
//	 * </p>
//	 *
//	 * @param lineIndex          line index the vertical indent is applied to, 0
//	 *                           based
...
//	 *                                     <ul>
//	 *                                     <li>ERROR_INVALID_ARGUMENT when the
//	 *                                     specified line index is invalid</li>
//	 *                                     </ul>
//	 * @since 3.109
//	 */
...
//			if ((initialTopIndex == 0) && (initialBottomIndex == (content.getLineCount() - 1))) { // not scrollable
//																									// editor
//				setCaretLocations();
//				redrawLines(lineIndex, getBottomIndex() - lineIndex + 1, true);
//			} else if (getFirstCaretLine() >= initialTopIndex && getFirstCaretLine() <= initialBottomIndex) { // caret
//																												// line
//																												// with
//																												// caret
//																												// mustn't
...
//	 * In the case of #setLineSpacingProvider(StyledTextLineSpacingProvider) is
//	 * customized, the line spacing are applied only for the lines which are not
//	 * managed by {@link StyledTextLineSpacingProvider}.
//	 *
...
//
//	/**
//	 * Sets the line spacing provider of the widget. The line spacing applies for
//	 * some lines with customized spacing or reset the customized spacing if the
//	 * argument is null.
//	 *
...
//				}
//				if (firstLine != -1) {
//					// call reset cache for the first line which have changed to recompute
//					// scrollbars
//					resetCache(firstLine, 0);
//				}
//			}
...
//	 * <p>
//	 * Should not be called if a <code>LineStyleListener</code> has been set since
//	 * the listener maintains the line attributes.
//	 * </p>
//	 * <p>
//	 * All line attributes are maintained relative to the line text, not the line
//	 * index that is specified in this method call. During text changes, when entire
...
//
//	/**
//	 * Sets the enabled state of the mouse navigator. When the mouse navigator is
//	 * enabled, the user can navigate through the widget by pressing the middle
//	 * button and moving the cursor.
//	 *
//	 * @param enabled if true, the mouse navigator is enabled, if false the mouse
//	 *                navigator is deactivated
//	 * @exception SWTException
//	 *                         <ul>
//	 *                         <li>ERROR_WIDGET_DISPOSED - if the receiver has been
//	 *                         disposed</li>
...
//		}
//		if (enabled) {
//			mouseNavigator = new MouseNavigator();
//		} else {
...
//
//	/**
//	 * Adjusts the maximum and the page size of the scroll bars to reflect content
...
//
//	/**
//	 * Sets the selection to the given position and scrolls it into view. Equivalent
//	 * to setSelection(start,start).
//	 *
...
//	 *                                     range is inside a multi byte line
//	 *                                     delimiter (and thus neither clearly in
//	 *                                     front of or after the line delimiter)
//	 *                                     </ul>
//	 */
...
//	 * </p>
//	 *
//	 * @param point x=selection start offset, y=selection end offset The caret will
//	 *              be placed at the selection start when x &gt; y.
//	 * @see #setSelection(int,int)
//	 * @exception SWTException
//	 *                                     <ul>
//	 *                                     <li>ERROR_WIDGET_DISPOSED - if the
...
//
//	/**
//	 * Sets the receiver's selection foreground color to the color specified by the
//	 * argument, or to the default system color for the control if the argument is
//	 * null.
//	 * <p>
...
//	 * </p>
//	 *
//	 * @param start     offset of the first selected character, start &gt;= 0 must
//	 *                  be true.
...
//			// different?
//			if (selection[0].x != start || selection[0].y != end || (length > 0 && selectionAnchor != selection[0].x)
...
//	 *                                     <ul>
//	 *                                     <li>ERROR_INVALID_ARGUMENT when either
//	 *                                     the start or the end of one selection
//	 *                                     range is inside a multi byte line
//	 *                                     delimiter (and thus neither clearly in
//	 *                                     front of or after the line
//	 *                                     delimiter)</li>
//	 *                                     <li>ERROR_INVALID_ARGUMENT when ranges
//	 *                                     are null or size isn't valid (not
//	 *                                     pair)</li>
//	 *                                     </ul>
//	 * @see #getSelectionRanges()
//	 * @since 3.117
...
//	 * clear an individual style, call setStyleRange with a StyleRange that has null
//	 * attributes.
//	 * </p>
//	 * <p>
//	 * Should not be called if a LineStyleListener has been set since the listener
...
//	 * @exception IllegalArgumentException
//	 *                                     <ul>
//	 *                                     <li>ERROR_INVALID_RANGE when the style
//	 *                                     range is outside the valid range (&gt;
//	 *                                     getCharCount())</li>
//	 *                                     </ul>
//	 */
...
//	 * The ranges array contains start and length pairs. Each pair refers to the
//	 * corresponding style in the styles array. For example, the pair that starts at
//	 * ranges[n] with length ranges[n+1] uses the style at styles[n/2]. The range
//	 * fields within each StyleRange are ignored. If ranges or styles is null, the
//	 * specified range is cleared.
//	 * </p>
//	 * <p>
...
//	 * @param start  offset of first character where styles will be deleted
//	 * @param length length of the range to delete styles in
//	 * @param ranges the array of ranges. The ranges must not overlap and must be in
//	 *               order.
//	 * @param styles the array of StyleRanges. The range fields within the
...
//	 * @exception IllegalArgumentException
//	 *                                     <ul>
//	 *                                     <li>ERROR_NULL_ARGUMENT when an element
//	 *                                     in the styles array is null</li>
//	 *                                     <li>ERROR_INVALID_RANGE when the number
//	 *                                     of ranges and style do not match
//	 *                                     (ranges.length * 2 == styles.length)</li>
//	 *                                     <li>ERROR_INVALID_RANGE when a range is
//	 *                                     outside the valid range (&gt;
//	 *                                     getCharCount() or less than zero)</li>
//	 *                                     <li>ERROR_INVALID_RANGE when a range
//	 *                                     overlaps</li>
//	 *                                     </ul>
//	 *
//	 * @since 3.2
...
//	 * <p>
//	 * All styles in the widget will be replaced with the given set of ranges and
//	 * styles. The ranges array contains start and length pairs. Each pair refers to
//	 * the corresponding style in the styles array. For example, the pair that
//	 * starts at ranges[n] with length ranges[n+1] uses the style at styles[n/2].
//	 * The range fields within each StyleRange are ignored. If either argument is
//	 * null, the styles are cleared.
//	 * </p>
//	 * <p>
//	 * Note: It is expected that the same instance of a StyleRange will occur
//	 * multiple times within the styles array, reducing memory usage.
...
//	/**
//	 *
//	 * @param referenceRanges former ranges, sorted by order and without
//	 *                        overlapping, typically returned
//	 *                        {@link #getRanges(int, int)}
...
//	 * cannot occur multiple times in the array of styles. If the same style
//	 * attributes, such as font and color, occur in multiple StyleRanges,
//	 * <code>setStyleRanges(int[], StyleRange[])</code> can be used to share styles
...
//	 * </p>
//	 *
//	 * @param ranges StyleRange objects containing the style information. The ranges
//	 *               should not overlap. The style rendering is undefined if the
...
//	 * @exception IllegalArgumentException
//	 *                                     <ul>
//	 *                                     <li>ERROR_NULL_ARGUMENT when the list of
//	 *                                     ranges is null</li>
//	 *                                     <li>ERROR_INVALID_RANGE when the last of
//	 *                                     the style ranges is outside the valid
//	 *                                     range (&gt; getCharCount())</li>
//	 *                                     </ul>
//	 *
//	 * @see #setStyleRanges(int[], StyleRange[])
//	 */
...
//
//	/**
//	 * Sets the receiver's tab list. Each value in the tab list specifies the space
//	 * in points from the origin of the document to the respective tab stop. The
//	 * last tab stop width is repeated continuously.
//	 *
...
//	 * @exception IllegalArgumentException
//	 *                                     <ul>
//	 *                                     <li>ERROR_INVALID_ARGUMENT - if a tab
//	 *                                     stop is negative or less than the
//	 *                                     previous stop in the list</li>
//	 *                                     </ul>
//	 *
...
//	 * contains more than one line, only the first line is rendered but the text is
//	 * stored unchanged. A subsequent call to getText will return the same text that
//	 * was set.
//	 * <p>
...
//	 *             were set using StyledText API are discarded. The current
...
//	 * <p>
//	 * The text limit specifies the amount of text that the user can type into the
//	 * widget.
//	 * </p>
//	 *
...
//	 * @exception IllegalArgumentException
//	 *                                     <ul>
//	 *                                     <li>ERROR_CANNOT_BE_ZERO when limit is
//	 *                                     0</li>
//	 *                                     </ul>
//	 */
...
//	 * <p>
//	 * The top index is the index of the line that is currently at the top of the
//	 * widget. The top index changes when the widget is scrolled. Indexing starts
//	 * from zero. Note: The top index is reset to 0 when new text is set in the
//	 * widget.
//	 * </p>
//	 *
//	 * @param topIndex new top index. Must be between 0 and getLineCount() - fully
...
//	 * <p>
//	 * The top point offset is the vertical SWT logical point offset of the widget.
//	 * The widget is scrolled so that the given SWT logical point position is at the
//	 * top. The top index is adjusted to the corresponding top line. Note: The top
//	 * point is reset to 0 when new text is set in the widget.
//	 * </p>
//	 *
//	 * @param pixel new top point offset. Must be between 0 and (getLineCount() -
...
//	 * <p>
//	 * It is the amount of blank space, in points, at the beginning of each wrapped
//	 * line. When a line wraps in several lines all the lines but the first one is
...
//	 * <p>
//	 * If the selection intersects with the replaced text, the selection is reset
//	 * and the caret moved to the end of the new text. If the selection is behind
//	 * the replaced text it is moved so that the same text remains selected. If the
//	 * selection is before the replaced text it is left unchanged.
//	 * </p>
//	 *
...
//		Arrays.stream(selection).filter(sel -> sel.y > startOffset).filter(sel -> sel.x < startOffset)
//				.forEach(sel -> internalRedrawRange(sel.x, startOffset - sel.x));
//		Arrays.stream(selection).filter(sel -> sel.y > startOffset)
